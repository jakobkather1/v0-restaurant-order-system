...
export async function validateRestaurantAdminByUsername(
  username: string,
  password: string
): Promise<{ valid: boolean; restaurantId?: number; slug?: string; error?: string }> {
  try {
    console.log("[v0] validateRestaurantAdminByUsername - Looking up username:", username)
    ...
    if (result.length === 0) {
      console.log("[v0] validateRestaurantAdminByUsername - Username not found")
      return { valid: false, error: "Benutzername nicht gefunden" }
    }

    const restaurant = result[0]
    console.log("[v0] validateRestaurantAdminByUsername - Restaurant found:", restaurant.id)

    if (!restaurant.admin_password_hash) {
      console.log("[v0] validateRestaurantAdminByUsername - No password set")
      return { valid: false, error: "Kein Passwort gesetzt" }
    }

    const isValid = await bcrypt.compare(password, restaurant.admin_password_hash)
    console.log("[v0] validateRestaurantAdminByUsername - Password valid:", isValid)
    ...
    console.error("[v0] validateRestaurantAdminByUsername - Error:", error)
    return { valid: false, error: "Interner Serverfehler" }
  }
}
